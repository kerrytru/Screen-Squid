...
my $day_traffic_per_login = $dbh->selectall_hashref("SELECT login, SUM(sizeinbytes) as s FROM scsq_quicktraffic WHERE date>$datestart AND date<$dateend GROUP BY login;", "login");

$datestart=$datenow[1]; #first day in month
$dateend=$datenow[2]; #first day next month
	
my $month_traffic_per_login = $dbh->selectall_hashref("SELECT login, SUM(sizeinbytes) as s FROM scsq_quicktraffic WHERE date>$datestart AND date<$dateend GROUP BY login;", "login");
...
$DaySumSizeTraffic = ($typeid == 0) ? $day_traffic_per_login->{$tableid}->{s} : die "ipaddress support is not implemented";
...
$MonthSumSizeTraffic = ($typeid == 0) ? $month_traffic_per_login->{$tableid}->{s} : die "ipaddress support is not implemented";
